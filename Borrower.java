...
public class Borrower {
	...
	private void book_return() {
		...
		try {
			...
			ResultSet rs1 = statement.executeQuery("select b.bookId,b.title from tbl_book_loans bl left join tbl_book b on bl.bookId=b.bookId where bl.cardNo='"+choice1+"' and dateIn IS NULL");
			while(rs1.next())
			{
				buk_id=rs1.getInt("bookId");
				b_title=rs1.getString("title");
				m1.put(c1,buk_id);
				System.out.println(c1+" "+b_title);
				c1++;
			}
			if(c1==1){
				System.out.println("You have returned all the books you had borrowed");
				menu1();
			}
			else{
			System.out.println("Select the book that you want to return ");
			Scanner sc = new Scanner(System.in);
			int boook_id=sc.nextInt();
			int book_id=m1.get(boook_id);
			...
			if(b=true){
				...
				//System.out.println(dateFormat.format(date));
					if(d1.equals(dateFormat.format(date)) || d1.after(date)){
						...
						//statement.executeUpdate("update tbl_book_loans set  where bookId='"+book_id+"' and cardNo='"+choice1+"'");
	...
	private void Checkout_books(String name) {
